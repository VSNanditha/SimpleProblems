...
def match_ends(words):
    return len(list(filter(lambda word: len(word) >= 2 and word[0] == word[-1], words)))
...
def front_x(words):
    x_list = sorted(list(filter(lambda word: word[0] == 'x', words)))
    other_list = sorted(list(filter(lambda word: word[0] != 'x', words)))
